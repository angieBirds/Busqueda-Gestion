	...
	<script>
		...
		<?php
			...
			if($m=="S"){ ?>
				...
				window.open("principal.php?m=A", "_self");
			}
		<?php }	?>
		} //fin function
	</script>
    ...
    if($m=="A" || $m=="C"){ 
		...
		<div class="center">
			<div  id="slider" class="registro">
				...
				<form name="registro" method="POST" action="principal.php"  enctype="multipart/formdata">
					<table>
						<tr>
	...
	<section id="sec">
		...
		while($data = mysqli_fetch_assoc($b)){ ?>
			<div id= "busqueda" class="center">
				<div id="foto"> 
					<?php	print"<a href='detalles.php?m=C&id_r=".$data['id_r']."'>"; ?>
					<td> 
					<?php 
					$ruta=opendir("fotos");
					if($ruta){
